...
- **PATCH** `/duties/schedules/{scheduleId}` — обновление статуса (только для воспитателей)
...
- **Статус**: Устанавливается `submitted` (автоматически)
...
- **PATCH** `/duties/reports/{reportId}` — изменение статуса (только для воспитателей)
...
created_at и updated_at - только в GET. created_at - добавить default now. Updated_at обновляется на текущее время при put, post.

информация о user, токен, его роли и так далее берутся из сервиса auth.
...
Reviewed_at и reviewed_by обновляются автоматически, видны только в GET
...
patch для обновление фотографии или категории
